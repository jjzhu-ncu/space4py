...
class DataAnalysis:
    ...
    def calc_CA(self):
        ...
        for time_slot in time_slots:
            ...
            # self.mysqlconn.execute_single('call Init_Balance(%s, %s)', (str(time_slot[0]),  str(1)))
            # self.init_balance('2016-03-01', BusinessType.CURR_DEP)
            # self.init_balance('2016-04-01', BusinessType.CURR_DEP)
    ...
    def loyalty(self):
        ...
        for row in join_result_df.collect():
            row_dic = row.asDict()
            ...
            insert_record.append((str(row_dic['CUST_ID']), str(row_dic['CUST_NO']), str(row_dic['CUST_NAM']),
                                  str(grade), str(datetime.datetime.strptime(end_time, '%Y-%m-%d')
                                                  .__format__('%Y%m%d')),
                                  str(curr_time)))
        # 如果列表中还有未处理的数据
    ...
    def aum_time_task(self):
        ...
        def ca_job():
            ...
            _time = str(datetime.datetime.now().__format__('%Y-%m-%d'))
            print('call Calc_balance(%s)' % _time)

            self.mysqlconn.execute_single('call Calc_balance(%s, %s)', (_time, str(1)))
            self._calc_bal_by_type(_time, 1)

        def fa_job():
            scheduler.enter(24*60*60, 0, fa_job)
            print(datetime.datetime.now())
            _time = str(datetime.datetime.now().__format__('%Y-%m-%d'))
            print('call Calc_FA(%s)' % _time)
            self.mysqlconn.execute_single('call Calc_balance(%s)', (_time, str(2)))
            self._calc_bal_by_type(_time, 2)
    ...
    def recommend(self):
        print('load data from mysql table t_CMMS_ACCOUNT_DETAIL')
        tran_df = self.load_from_mysql('core', 't_CMMS_ACCOUNT_DETAIL').select('ACC_NO15', 'TRAN_CHL')
        tran_df = tran_df.filter(tran_df.ACC_NO15 != '').filter(tran_df.TRAN_CHL != '')

        reduce_rdd = tran_df.map(lambda row: (row.asDict()['ACC_NO15'], set(str([row.asDict()['TRAN_CHL']]))))\
            .filter(lambda a: a.__len__() == 2)\
            .reduceByKey(lambda a, b: a.union(b))\
            .filter(lambda e: e[1].__len__() > 1)\
            .map(lambda e: e[1])

        reduce_rdd.cache()
        model = FPGrowth.train(reduce_rdd, minSupport=0.1, numPartitions=10)
    ...
    def init_balance(self, ctime, tp):
        ...
        try:
            ...
            # AUM计算结果的hdfs输出路径
            hdfs_save_path = '%s/jjzhu/test/result/_%s/%s' % (self.hdfs_root_url, str(tp.value), cdate.strftime('%Y_%m'))
            # 计算AUM原文件的hdfs输入路径
            ...
            def time_incr(row):
                row_dict = row.asDict()
                return Row(CUST_NO=row_dict['CUST_NO'],
                           ACCT_NO15=row_dict['ACCT_NO15'],
                           CURR_BAL=Decimal(row_dict['CURR_BAL']),
                           TIME=int(row_dict['TIME'] + 1),
                           CUR=row_dict['CUR'],
                           CURR_DATE=row_dict['CURR_DATE'],
                           BUSI_TYPE=str(row_dict['BUSI_TYPE']))
            ...
            while end_date.month == cdate.month:
                ...
                curr_bal_rdd = curr_bal_df.filter(curr_bal_df.BUSI_TYPE == tp.value).map(change_row2)
